...
start_the_google_cloud_platform_worker_machines()
{
    ...
    for i in $(seq $number_of_workers); do
        gcloud compute instances create worker-$fuzzer_input_program-$i --zone=northamerica-northeast2-a --machine-type=e2-micro --image-family=debian-11 --image-project=debian-cloud
    done
    sleep 5
}
...
start_the_AFL_fuzzer_on_the_google_cloud_platform_worker_machines()
{
    ...
    do
       ...
       then
          ...
          if [[ "$name" == *"worker-$fuzzer_input_program"* ]]
          then
             echo "Worker instance - $name"
	     sleep 3
